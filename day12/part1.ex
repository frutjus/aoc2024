...
AoC.iterate_until(
    {initial_grid, [], %{symbol: nil, pots: MapSet.new(), area: 0, perimeter: 0}, [], 0},
    fn {unassigned_pots, found_pots, current_region, regions, step} ->
        if empty?(found_pots) do
            if empty?(unassigned_pots) do
                ...
                new_regions = if empty?(current_region.pots) do regions else [current_region | regions] end
...
|> map(fn %{area: a, perimeter: p} -> a * p end)
|> sum()
|> IO.inspect()