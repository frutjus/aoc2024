...
{[{initial_position, "@"}], initial_tiles} =
    ...
    |> then(fn {pos, tiles} -> {pos,
        map(tiles, fn {coord, ch} -> {coord,
            case ch do
                "O" -> :box
                "#" -> :wall
            end
...
defmodule Day15 do
    def simulate_step(move, {_, {r,c} = position, tiles}) do
        ...
        case tiles[new_position] do
            ...
            :box -> case simulate_step(move, {nil, new_position, tiles}) do
    ...
    for r <- 0..row_max do
        for c <- 0..col_max do
            IO.write(case all_tiles[{r,c}] do
                ...
                :box   -> IO.ANSI.color(2) <> "O" <> IO.ANSI.reset()
