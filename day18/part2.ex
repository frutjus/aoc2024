...
mode = :actual
{file_path, x_max, y_max} =
    case mode do
        :sample -> {"day18/sample.txt", 6, 6}
        :actual -> {"day18/input.txt", 70, 70}
    end
...
AoC.heads(all_corrupted)
|> with_index()
|> drop_while(fn {corrupted, i} ->
    IO.write("#{i}: ")
    path = find_paths.(MapSet.new(corrupted))
    if path == nil do
        false
    else
        IO.puts("#{elem(path,1)}")
